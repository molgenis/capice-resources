...
run_vep(){
	echo "running vep"
	for file in ${WORKDIR}/poc/data/*.vcf.gz; do bash ${CAPICE_RESOURCES}/utility_scripts/slurm_run_vep.sh -p ${VIP_DIR} -i "${file}" -g -o "${file%.vcf.gz}_vep.vcf.gz"; done
	 mv "${WORKDIR}/poc/data/train_input_raw_vep.vcf.gz" "${WORKDIR}/poc/data/train_input_annotated.vcf.gz"
	 mv "${WORKDIR}/poc/data/predict_input_raw_vep.vcf.gz" "${WORKDIR}/poc/data/predict_input.vcf.gz"
	echo "finished running vep"
}

conversion_tool(){
	bash "${CAPICE}/scripts/convert_vep_vcf_to_tsv_capice.sh" -t -i "${WORKDIR}/poc/data/train_input_annotated.vcf.gz" -o "${WORKDIR}/poc/data/train_input_annotated.tsv.gz" -p "${BCFTOOLS_SIF}"
}
...
process_vep(){
	...
	wget -P "${WORKDIR}/poc/data/" https://research.nhgri.nih.gov/CGD/download/txt/CGD.txt.gz
	module load Python/3.10.4-GCCcore-11.3.0-bare
	source "${CAPICE_RESOURCES}/venv/bin/activate"
	process-vep -g "${WORKDIR}/poc/data/CGD.txt.gz" -f "${CAPICE}/resources/train_features.json" -t "${WORKDIR}/poc/data/train_input_annotated.tsv.gz" -o "${WORKDIR}/poc/data/processed/"
	deactivate
	module purge
...
#SBATCH --output=${WORKDIR}/poc/capice_train.log
#SBATCH --error=${WORKDIR}/poc/capice_train.err
#SBATCH --time=23:59:00
#SBATCH --cpus-per-task=8
#SBATCH --mem=25gb
#SBATCH --nodes=1
...
module load Python/3.10.4-GCCcore-11.3.0-bare
source ${CAPICE}/venv/bin/activate
capice -v train -t 8 -i ${WORKDIR}/poc/data/processed/train_test.tsv.gz \
-e ${CAPICE}/resources/train_features.json \
-o ${WORKDIR}/poc/xgb_booster_poc.ubj
...
vcf_to_tsv(){
	...
	bash ${CAPICE}/scripts/convert_vep_vcf_to_tsv_capice.sh -p "${BCFTOOLS_SIF}" -t -i "${WORKDIR}/poc/data/predict_input.vcf.gz" -o "${WORKDIR}/poc/data/predict_input.tsv.gz"
	bash ${CAPICE}/scripts/convert_vep_vcf_to_tsv_capice.sh -p "${BCFTOOLS_SIF}" -t -i "${WORKDIR}/poc/data/breakends_vep.vcf.gz" -o "${WORKDIR}/poc/data/breakends_vep.tsv.gz"
	bash ${CAPICE}/scripts/convert_vep_vcf_to_tsv_capice.sh -p "${BCFTOOLS_SIF}" -t -i "${WORKDIR}/poc/data/edge_cases_vep.vcf.gz" -o "${WORKDIR}/poc/data/edge_cases_vep.tsv.gz"
	bash ${CAPICE}/scripts/convert_vep_vcf_to_tsv_capice.sh -p "${BCFTOOLS_SIF}" -t -i "${WORKDIR}/poc/data/symbolic_alleles_vep.vcf.gz" -o "${WORKDIR}/poc/data/symbolic_alleles_vep.tsv.gz"
...
copy_to_capice(){
	...
	cp "${WORKDIR}/poc/data/edge_cases_vep.tsv.gz" "${CAPICE}/tests/resources/"
	cp "${WORKDIR}/poc/data/symbolic_alleles_vep.tsv.gz" "${CAPICE}/tests/resources/"
