...
class VEPProcesser:
    ...
    def drop_genes_empty(data: pd.DataFrame) -> None:
        ...
        data.drop(index=data[data[GlobalEnums.SYMBOL.value].isnull()].index, inplace=True)

    @staticmethod
    def process_grch38(data: pd.DataFrame):
        print('Processing GRCh38.')
        data[GlobalEnums.CHROM.value] = \
        data[GlobalEnums.CHROM.value].str.split('chr', expand=True)[1]
        y = np.append(np.arange(1, 23).astype(str), ['X', 'Y', 'MT'])
        data.drop(data[~data[GlobalEnums.CHROM.value].isin(y)].index, inplace=True)
    ...
    def drop_heterozygous_variants_in_ar_genes(data: pd.DataFrame, cgd: list):
        ...
        data.drop(
            data[
                ...
                (data[GlobalEnums.SYMBOL.value].isin(cgd))
                ].index, inplace=True
        )
    ...
    def drop_variants_incorrect_label_or_weight(data: pd.DataFrame):
        ...
        data.drop(
            index=data[~data[GlobalEnums.BINARIZED_LABEL.value].isin([0.0, 1.0])].index,
            inplace=True
        )
        data.drop(
